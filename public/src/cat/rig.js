/* ── src/cat/rig.js ──────────────────────────────────────────────────
   The cat's skeleton and the container it arrives in.

   Adapted from `src/scenes/cat/rig.js` of the project that baked
   `assets/cat.bin` (the "fes" scene pack). Trimmed to what this
   renderer needs: the parser, the rig, and the three matrix operations
   it is built out of. `modelMatrix` is gone — the cat is placed with an
   orthographic side camera instead of a yaw and a world position.
   Everything else is the original, comments included, because the
   comments are what say *why* the maths is shaped this way.

   The model is animated as a rigid hierarchy: every part is a whole
   mesh swinging on a joint, and there is not one skinning weight in it.
   A bone is a position, a rotation and a scale, and a world matrix is
   the product down the chain. Twenty-three of them, per character, per
   frame — about 3,000 floating-point operations, which is why the rig
   can afford to be live while the triangles are the thing to watch.
   ------------------------------------------------------------------ */
 ...
 * stored once and only the colour block repeats.
 */
...
export class Rig {
   ...
   */
  update() {
    const { matrices, offset, parent, _composed, _local } = this;

    for (let i = 0; i < this.count; i++) {
      compose(_composed, this.position, this.rotation, this.scale, i, this.order[i]);
      multiplyInto(_local, 0, offset, i * 16, _composed, 0);

      const p = parent[i];
      if (p < 0) matrices.set(_local, i * 16);
      else multiplyInto(matrices, i * 16, matrices, p * 16, _local, 0);
    }
    return matrices;
  }
}
